...
# American forms that are also surnames. Capitalised, they are names, not
# spellings: radicofani-guide lists the poet Thomas Gray among the town's
# visitors three times. The colour at a sentence start is missed — accepted.
SURNAMES = {"gray"}
...
def findings_for(article):
    ...
    for m in WORD.finditer(text):
        ...
        if _inside(m.start(), quotes):
            continue
        found = m.group(0)
        if found[0].isupper() and found.lower() in SURNAMES:
            continue
