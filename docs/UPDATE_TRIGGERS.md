...
| 37 | **Production deploy freeze, August 2026** — every Cloudflare build failed 15–18 Aug: `CI=true` turns CRA lint warnings into errors, and a `react-hooks/exhaustive-deps` warning in Blog.jsx (introduced with the 15 Aug runtime publish filter) killed each build, while GitHub CI stayed green because e2e built with `CI=false` | resolved | Cloudflare Pages dashboard; issues #108/#110 | site-wide | **RESOLVED 18 Aug 2026**: PR #110 fixed the warning, set e2e to build with `CI=true` like Cloudflare, and bumped Node to 20; production deploys green since 03:46 UTC and #108 auto-closed. Guarded from now on by the daily 00:05 WIB publish trigger + truthful smoke (#109). **Addendum 21 Aug**: the trigger itself then failed at startup on every run 17-20 Aug (reusable-workflow permission mismatch, fixed #121, dispatch-verified green); its first honest scheduled run is the night of 21 Aug |date triggers — the editorial maintenance registry
...
| 35 | **The full list of seven official contrada rivalries** (we confirmed that seven recognised pairs exist and verified several, but did not enumerate all seven from a primary source) | any time palio.comune.siena.it "Rivalità e alleanze" is reachable | palio.comune.siena.it | `siena-contrade-guide` | Replace the partial list with the complete seven and remove the flagged gap |  |
| 36 | **Contrada patron saints, titular feast dates and the baptism rite** (unestablished; the Chiocciola-1949 revival story is widely repeated and unconfirmed) | rolling | comune.siena.it Glossario, individual contrada sites | `siena-contrade-guide` | These govern when each contrada celebrates and baptises — the missing piece that would let the page carry a usable annual calendar |  |
| 37 | **Production deploy freeze, August 2026** — every Cloudflare build failed 15–18 Aug: `CI=true` turns CRA lint warnings into errors, and a `react-hooks/exhaustive-deps` warning in Blog.jsx (introduced with the 15 Aug runtime publish filter) killed each build, while GitHub CI stayed green because e2e built with `CI=false` | resolved | Cloudflare Pages dashboard; issues #108/#110 | site-wide | **RESOLVED 18 Aug 2026**: PR #110 fixed the warning, set e2e to build with `CI=true` like Cloudflare, and bumped Node to 20; production deploys green since 03:46 UTC and #108 auto-closed. Guarded from now on by the daily 00:05 WIB publish trigger + truthful smoke (#109). **Addendum 21 Aug**: the trigger itself then failed at startup on every run 17-20 Aug (reusable-workflow permission mismatch, fixed #121, dispatch-verified green); its first honest scheduled run is the night of 21 Aug |
| 38 | **Monteriggioni wall-walk tariff and section openings** (currently the south section only, with the *Monteriggioni in Arme* exhibition and the MAM at Abbadia a Isola on one combined ticket; the commune's current price was not confirmable — third-party guides list a few euros, and `tuscany-on-screen` still carries an older €4/€3.50 figure with its own caveat) | seasonal republication of tariffs; any reopening of further wall sections | monteriggioniturismo.it tariff pages, tourist office +39 0577 304834 | `monteriggioni-guide`, `tuscany-on-screen-film-locations-2026` | Pin the price in both articles, state which walkway sections are open, and refresh the seasonal hours |  |
