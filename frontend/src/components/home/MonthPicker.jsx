          ...
          {MONTHS.map((m) => (
            <button
              ...
              className={["px-4 py-2 rounded-full border text-sm font-medium transition-all",
                m === month
                  ? "border-[#C65A3A] bg-[#A84A2E] text-white shadow-sm"
                  : monthGuide(m)
                    ? "border-[#C65A3A]/40 bg-white text-[#2C211B] hover:border-[#C65A3A]"
                    : "border-[#e8dfd4] bg-white/70 text-[#657143] hover:border-[#C65A3A]/40",
              ].join(" ")}
            >
              {m}
            </button>
        ...
        <div className="mt-6 rounded-3xl border border-[#F5EDE3] bg-white p-6 sm:p-8 shadow-sm">
          <div className="flex flex-col-reverse md:flex-row gap-6 md:items-center">
            <div className="flex-1 min-w-0">
              {guide ? (
                <>
                  <p className="text-xs font-semibold uppercase tracking-widest text-[#657143]">Month guide</p>
                  <h3 className="mt-2 font-serif text-2xl text-[#2C211B]">{guide.title}</h3>
                  <p className="mt-3 text-sm text-[#657143] leading-relaxed max-w-3xl">{guide.excerpt}</p>
                  <Link
                    to={guide.canonicalPath || `/blog/${guide.slug}`}
                    className="mt-5 inline-flex items-center gap-2 text-sm font-medium text-[#A84A2E] hover:underline"
                  >
                ...
                <>
                  <p className="text-xs font-semibold uppercase tracking-widest text-[#657143]">No dedicated {month} guide yet</p>
                  <p className="mt-3 text-sm text-[#657143] leading-relaxed max-w-3xl">
                  ...
                  <Link to="/travel-tools" className="mt-5 inline-flex items-center gap-2 text-sm font-medium text-[#A84A2E] hover:underline">
