...
// Connects the destination page to the My Trip layer: visitors with a saved
// Siena plan see their countdown (and the verified Palio warning when their
// dates overlap); everyone else gets a single planning CTA.
export default function SienaPlanStrip() {
  ...
  const sienaPlan = plan && plan.destination === "Siena" ? plan : null;
  const days = sienaPlan ? daysUntilTrip(sienaPlan) : null;
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-4">
            <div className="flex items-center gap-3 flex-1 min-w-0">
              ...
              <p className="font-serif text-lg text-[#2C211B] leading-tight">
                {sienaPlan
                  ? days !== null && days > 0
                    ? `${days} ${days === 1 ? "day" : "days"} to your Siena trip`
                    : days === 0
                      ? "Your Siena trip starts today"
                      : `Your Siena plan: ${sienaPlan.trip_length} ${sienaPlan.trip_length === 1 ? "night" : "nights"}`
                  : "Planning a Siena trip? Set it once and every tool follows."}
              </p>
            </div>
            <Link
              to={sienaPlan ? "/travel-tools?tool=trip-sheet" : "/travel-tools"}
            ...
            >
              {sienaPlan ? "Open your Trip Sheet" : "Plan your Siena trip"} <ArrowRight className="w-4 h-4" />
            </Link>
          </div>
          {sienaPlan && tripOverlapsPalio(sienaPlan) && (
